...
- **Land dev adds detail.** Land development tracks extra attributes per contact: Type (Land Owner, Developer, Broker), Office Address, Notes, City, State, Date of Contact, Follow-up Timeframe (days), **ReminderToEmail** (where to send “follow-up due” reminders to the dashboard user), and follow-up reminders. These are stored in a **land-dev extension** linked to core.contacts.
- **Two flows:**
...
- **core.contacts** = **core.Person**; **ContactId** = **PersonId**. Individuals only.
- **Extension:** **pipeline.LandDevelopmentContactExtension** (schema: `schema/add_land_development_contact_extension.sql`). Currently includes: OfficeAddress, Type, Notes, City, State, DateOfContact, FollowUpTimeframeDays, CreatedAt, ModifiedAt. **ReminderToEmail is not yet in schema or API**—add it if you implement scheduled “reminder to dashboard user” emails.
- **List/Get/Create/Update/Delete:** Implemented as in §3. Delete removes extension only; response `"Land development attributes removed"`.
- **Send reminder:** Single (`contactId`/`email`/`message`) and **batch** (`contactIds` + optional `email`/`message`) implemented; response single `{ message: "Reminder sent" }` or batch `{ sent, failed }`. STOA HTML, same SMTP env.
- **Scheduled reminders** (§3): Job that emails **ReminderToEmail** when NextFollowUpDate is reached is **not implemented**; requires ReminderToEmail column and a cron/scheduled task.
