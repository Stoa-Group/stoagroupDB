 ...
 * records. Matches filenames to deals by keyword rules.
 ...
 * Prereq: Gulf Coast deals seeded (db:seed-site-tracking-gulf-coast), attachment table exists.
 */
...
const FILE_TO_PROJECT_NAME: { patterns: string[]; projectName: string }[] = [
  // Identified / Under Review / LOI / Under Contract
  ...
  // Rejected / other Gulf Coast sites
...
async function main() {
  ...
  if (matchedNoDeal.length > 0) {
    console.log(`Matched to a deal name but deal not in DB (seed Gulf Coast first): ${matchedNoDeal.length}`);
