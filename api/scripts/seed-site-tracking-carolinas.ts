#!/usr/bin/env ts-node
/**
 * Seed Site Tracking Worksheet - Carolinas + East GA data into the database
 *
 * Reads data/Site Tracking Worksheet - Carolinas + East GA .csv and creates
 * core.Project + pipeline.DealPipeline for each row. Region is set to
 * "Carolinas" for all records. City column maps to core.Project.City.
 *
 * Usage: npm run db:seed-site-tracking-carolinas
...
const REGION_CAROLINAS = 'Carolinas';
...
async function main() {
  ...
  console.log('Seeding Site Tracking Worksheet - Carolinas + East GA (Region = Carolinas)\n');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const listingStatus = get(row, listedIdx);
    ...
    try {
      ...
      if (existing.recordset.length > 0) {
        ...
        await pool.request()
          ...
          .input('Region', sql.NVarChar(50), REGION_CAROLINAS)
    ...
    try {
      ...
      created++;
      if (created <= 15 || created % 50 === 0) console.log(`✅ ${status}: ${site} (${city || '—'})`);
    } catch (e: any) {
      errors++;
  ...
  await pool.close();
  console.log('\n---');
  console.log(`Created/updated: ${created} (Region = ${REGION_CAROLINAS})`);
