...
/** Find first row key whose name (lowercase) contains all includeSubstrings and none of excludeSubstrings; return that key's value. Tries all matching keys and returns first non-empty value so Domo columns with varying names (e.g. budgeted occupancy) still seed when one alias is empty. */
...
export async function syncMMRData(rows: Record<string, unknown>[], replace = true): Promise<number> {
  ...
  try {
    const n = await batchInsert(tx, T_MMR, MMR_COLS, rows, (row) => [
      ...
      str(getValWithOverrides(MMR_ALIAS, 'Week3EndDate', row, 'Week3 End Date', 'Week 3 End Date')), num(getValWithOverrides(MMR_ALIAS, 'Week3MoveIns', row, 'Week3: Move Ins', 'Week 3 Move Ins')), num(getValWithOverrides(MMR_ALIAS, 'Week3MoveOuts', row, 'Week3: Move Outs', 'Week 3 Move Outs')), num(getValWithOverrides(MMR_ALIAS, 'Week3OccUnits', row, 'Week3: Occ Units', 'Week 3 Occ Units')), num(getValWithOverrides(MMR_ALIAS, 'Week3OccPercent', row, 'Week3: Occ %', 'Week 3 Occ Percent')), str(getValWithOverrides(MMR_ALIAS, 'Week4EndDate', row, 'Week 4 End Date', 'Week4 End Date')), num(getValWithOverrides(MMR_ALIAS, 'Week4MoveIns', row, 'Week 4 Move Ins', 'Week 4 Move-Ins')), num(getValWithOverrides(MMR_ALIAS, 'Week4MoveOuts', row, 'Week 4 Move Outs', 'Week 4 Move-Outs')), num(getValWithOverrides(MMR_ALIAS, 'Week4OccUnits', row, 'Week 4 Occ Units')), num(getValWithOverrides(MMR_ALIAS, 'Week4OccPercent', row, 'Week 4 Occ Percent', 'Week 4 Occ %')), str(getValWithOverrides(MMR_ALIAS, 'Week7EndDate', row, 'Week 7 End Date', 'Week7 End Date')), num(getValWithOverrides(MMR_ALIAS, 'Week7MoveIns', row, 'Week 7 Move Ins', 'Week 7 Move-Ins')), num(getValWithOverrides(MMR_ALIAS, 'Week7MoveOuts', row, 'Week 7 Move Outs', 'Week 7 Move-Outs')), num(getValWithOverrides(MMR_ALIAS, 'Week7OccUnits', row, 'Week 7 Occ Units')), num(getValWithOverrides(MMR_ALIAS, 'Week7OccPercent', row, 'Week 7 Occ Percent', 'Week 7 Occ %')),       num(getValWithOverrides(MMR_ALIAS, 'InServiceUnits', row, 'In Service Units', 'In-Service Units')), num(getValWithOverrides(MMR_ALIAS, 'T12LeaseBreaks', row, 'T-12 Lease Breaks', 'T12 Lease Breaks', 'T12 Leases Break')), num(getValWithOverrides(MMR_ALIAS, 'BudgetedOccupancyCurrentMonth', row, 'Budgeted Occupancy (Current Month)', 'Budgeted Occupancy(Current Month)', 'Budgeted Occupancy Current Month', 'Budgeted Occupancy (This Month)', 'BudgetedOccupancyCurrentMonth', 'Budgeted Occ Current Month') ?? getValBySubstrings(row, ['budgeted', 'occupancy', 'current month'], ['%', 'percent']) ?? getValBySubstrings(row, ['budgeted', 'occupancy', 'this month'], ['%', 'percent']) ?? getValBySubstrings(row, ['budgeted', 'occupancy', 'month'], ['%', 'percent'])), num(getValWithOverrides(MMR_ALIAS, 'BudgetedOccupancyPercentCurrentMonth', row, 'Budgeted Occupancy % (Current Month)', 'Budgeted Occupancy %(Current Month)', 'Budgeted Occupancy % Current Month', 'Budgeted Occupancy Percent Current Month', 'BudgetedOccupancyPercentCurrentMonth', 'Budgeted Occ % Current Month') ?? getValBySubstrings(row, ['budgeted', 'occupancy', '%']) ?? getValBySubstrings(row, ['budgeted', 'occupancy', 'percent'])), num(getValWithOverrides(MMR_ALIAS, 'BudgetedLeasedPercentCurrentMonth', row, 'Budgeted Leased % (Current Month)', 'Budgeted Leased % Current Month', 'Budgeted Leased Percent Current Month') ?? getValBySubstrings(row, ['budgeted', 'leased', '%']) ?? getValBySubstrings(row, ['budgeted', 'leased', 'percent'])), num(getValWithOverrides(MMR_ALIAS, 'BudgetedLeasedCurrentMonth', row, 'Budgeted Leased (Current Month)', 'Budgeted Leased Current Month', 'Budgeted Leased (This Month)', 'BudgetedLeasedCurrentMonth') ?? getValBySubstrings(row, ['budgeted', 'leased', 'current month'], ['%']) ?? getValBySubstrings(row, ['budgeted', 'leased', 'this month'], ['%'])),
...
export async function syncUnitByUnitTradeout(rows: Record<string, unknown>[], replace = true): Promise<number> {
  ...
  try {
    const n = await batchInsert(tx, T_UTRADE, UTRADE_COLS, rows, (row) => [
      ...
      str(getValWithOverrides(UTRADE_ALIAS, 'UnitDetailsUnitType', row, 'Unit Details | Unit Type', 'Unit Type', 'UnitDetails UnitType', 'Unit Type Details')),
      str(getValWithOverrides(UTRADE_ALIAS, 'UnitDetailsBuilding', row, 'Unit Details | Building', 'Building', 'UnitDetails Building', 'Unit Building')),
      str(getValWithOverrides(UTRADE_ALIAS, 'UnitDetailsUnit', row, 'Unit Details | Unit', 'Unit', 'Unit Number', 'Unit #', 'UnitDetails Unit')),
      num(getValWithOverrides(UTRADE_ALIAS, 'UnitDetailsSqFt', row, 'Unit Details | Sq Ft', 'Sq Ft', 'Square Feet', 'UnitDetails SqFt', 'SqFt')),
      str(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseRateType', row, 'Current Lease | Rate Type', 'Current Lease Rate Type', 'Rate Type', 'Lease Rate Type')),
      str(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseLeaseType', row, 'Current Lease | Lease Type', 'Current Lease Lease Type', 'Lease Type', 'Current Lease Type')),
      str(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseAppSignedDate', row, 'Current Lease | App Signed Date', 'App Signed Date', 'Application Signed Date', 'Current Lease App Signed Date')),
      str(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseLeaseStart', row, 'Current Lease | Lease Start', 'Lease Start', 'Current Lease Start', 'Current Lease Lease Start')),
      str(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseLeaseEnd', row, 'Current Lease | Lease End', 'Lease End', 'Current Lease End', 'Current Lease Lease End')),
      num(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseTerm', row, 'Current Lease | Term', 'Current Lease Term', 'Term', 'Lease Term')),
      num(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeasePrem', row, 'Current Lease | Prem', 'Premium', 'Prem')),
      num(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseGrossRent', row, 'Current Lease | Gross Rent', 'Current Lease Gross Rent', 'Gross Rent', 'GrossRent')),
      num(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseConc', row, 'Current Lease | Conc', 'Current Lease Conc', 'Concession', 'Conc')),
      num(getValWithOverrides(UTRADE_ALIAS, 'CurrentLeaseEffRent', row, 'Current Lease | Eff Rent', 'Effective Rent', 'Eff Rent', 'EffRent')),
      str(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseRateType', row, 'Previous Lease | Rate Type', 'Previous Lease Rate Type', 'Prev Rate Type')),
      str(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseLeaseStart', row, 'Previous Lease | Lease Start', 'Previous Lease Start', 'Previous Lease Lease Start')),
      str(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseScheduledLeaseEnd', row, 'Previous Lease | Scheduled Lease End', 'Previous Lease Scheduled Lease End', 'Scheduled Lease End', 'Prev Scheduled End')),
      str(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseActualLeaseEnd', row, 'Previous Lease | Actual Lease End', 'Previous Lease Actual Lease End', 'Actual Lease End', 'Prev Actual End')),
      num(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseTerm', row, 'Previous Lease | Term', 'Previous Lease Term', 'Prev Term')),
      num(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeasePrem', row, 'Previous Lease | Prem', 'Previous Lease Prem', 'Prev Prem')),
      num(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseGrossRent', row, 'Previous Lease | Gross Rent', 'Previous Lease Gross Rent', 'Prev Gross Rent')),
      num(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseConc', row, 'Previous Lease | Conc', 'Previous Lease Conc', 'Prev Conc')),
      num(getValWithOverrides(UTRADE_ALIAS, 'PreviousLeaseEffRent', row, 'Previous Lease | Eff Rent', 'Previous Lease Eff Rent', 'Prev Eff Rent')),
      ...
      num(getValWithOverrides(UTRADE_ALIAS, 'TradeOutPercentage', row, 'Trade Out %', 'Trade Out Percentage', 'TradeOut %')),
      num(getValWithOverrides(UTRADE_ALIAS, 'TradeOutAmount', row, 'Trade Out $', 'Trade Out Amount', 'Trade Out Amt')),
...
export async function syncPortfolioUnitDetails(rows: Record<string, unknown>[], replace = true): Promise<number> {
  ...
  try {
    const n = await batchInsert(tx, T_PUD, PUD_COLS, rows, (row) => [
      ...
      str(getValWithOverrides(PUD_ALIAS, 'UnitLeaseStatus', row, 'Unit/Lease Status', 'Unit Lease Status', 'Lease Status', 'Status', 'Unit Status')),
      str(getValWithOverrides(PUD_ALIAS, 'ResidentNameExternalTenantID', row, 'Resident Name/External Tenant ID', 'Resident Name', 'External Tenant ID', 'Tenant ID', 'Resident', 'Tenant')),
      ...
      str(getValWithOverrides(PUD_ALIAS, 'BATCHLASTRUN', row, '_BATCH_LAST_RUN_', 'Batch Last Run', 'Batch Timestamp', 'Last Run', 'Report Time')),
    ], replace, PUD_KEYS);
    await tx.commit();
    return n;
  } catch (e) {
...
export async function syncUnits(rows: Record<string, unknown>[], replace = true): Promise<number> {
  ...
  try {
    const n = await batchInsert(tx, T_UNITS, UNITS_COLS, rows, (row) => [
      ...
      str(getValWithOverrides(UNITS_ALIAS, 'BldgUnit', row, 'Bldg - Unit', 'Bldg Unit', 'Unit', 'Unit #', 'Unit Number', 'Building Unit')),
      ...
      num(getValWithOverrides(UNITS_ALIAS, 'Monthlygrossrent', row, 'Monthly gross rent', 'Monthly Gross Rent', 'Gross Rent', 'Rent', 'Monthly Rent')),
      str(getValWithOverrides(UNITS_ALIAS, 'Concessions', row, 'Concessions', 'Concession')),
...
export async function syncUnitMix(rows: Record<string, unknown>[], replace = true): Promise<number> {
  ...
  try {
    const n = await batchInsert(tx, T_UNITMIX, UNITMIX_COLS, rows, (row) => [
      ...
      num(getValWithOverrides(UNITMIX_ALIAS, 'PercentOccupied', row, '% Occupied', 'Percent Occupied', 'Occupied %')),
      num(getValWithOverrides(UNITMIX_ALIAS, 'percentLeased', row, '% Leased', 'Percent Leased', 'percent Leased')),
      num(getValWithOverrides(UNITMIX_ALIAS, 'GrossOfferedRent30days', row, 'Gross Offered Rent ( 30 days)', 'Gross Offered Rent 30 days', 'Gross Offered Rent 30 Days', 'Offered Rent 30d')),
      num(getValWithOverrides(UNITMIX_ALIAS, 'GrossInPlaceRent', row, 'Gross In Place Rent', 'In Place Rent', 'In-Place Rent')),
      num(getValWithOverrides(UNITMIX_ALIAS, 'GrossRecentExecutedRent60days', row, 'Gross Recent Executed Rent (60 days)', 'Gross Recent Executed Rent 60 days', 'Recent Executed Rent 60d', 'Executed Rent 60d')),
...
export async function syncPricing(rows: Record<string, unknown>[], replace = true): Promise<number> {
  ...
  try {
    const n = await batchInsert(tx, T_PRICING, PRICING_COLS, rows, (row) => [
      ...
      num(getValWithOverrides(PRICING_ALIAS, 'CapacityActualUnits', row, 'Capacity | Actual | Units', 'Capacity - Actual Units')),
      num(getValWithOverrides(PRICING_ALIAS, 'CapacitySustainablePercentage', row, 'Capacity | Sustainable | %', 'Capacity - Sustainable Percentage', 'Capacity - Sustainable %')),
      num(getValWithOverrides(PRICING_ALIAS, 'CapacitySustainableUnits', row, 'Capacity | Sustainable | Units', 'Capacity - Sustainable Units')),
      num(getValWithOverrides(PRICING_ALIAS, 'CurrentInPlaceLeases', row, 'Current | In Place | Leases', 'Current In Place Leases')),
      num(getValWithOverrides(PRICING_ALIAS, 'CurrentInPlaceOcc', row, 'Current | In Place | Occ', 'Current In Place Occ')),
      num(getValWithOverrides(PRICING_ALIAS, 'CurrentForecastLeases', row, 'Current |Forecast | Leases', 'Current Forecast Leases')),
      num(getValWithOverrides(PRICING_ALIAS, 'CurrentForecastOcc', row, 'Current |Forecast | Occ', 'Current Forecast Occ')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendedForecastLeases', row, 'Recommended  Forecast | Leases', 'Recommended Forecast Leases')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendedForecastOcc', row, 'Recommended  Forecast | Occ', 'Recommended Forecast Occ')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendedForecastChg', row, 'Recommended  Forecast | Chg', 'Recommended Forecast Chg')),
      str(getValWithOverrides(PRICING_ALIAS, 'YesterdayDate', row, 'Yesterday | Date', 'Yesterday Date')),
      num(getValWithOverrides(PRICING_ALIAS, 'YesterdayRent', row, 'Yesterday | Rent', 'Yesterday Rent')),
      num(getValWithOverrides(PRICING_ALIAS, 'YesterdayPercentage', row, 'Yesterday | %', 'Yesterday Percentage')),
      num(getValWithOverrides(PRICING_ALIAS, 'AmenityNormModelRent', row, 'Amenity Norm | Model Rent', 'Amenity Norm Model Rent')),
      num(getValWithOverrides(PRICING_ALIAS, 'AmenityNormAmenAdj', row, 'Amenity Norm | Amen Adj', 'Amenity Norm Amen Adj')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendationsRecommendedEffRent', row, 'Recommendations |  Recommended Eff | Rent', 'Recommendations - Recommended Eff Rent')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendationsRecommendedEffPercentage', row, 'Recommendations |  Recommended Eff | %', 'Recommendations - Recommended Eff Percentage')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendationsChangeRent', row, 'Recommendations | Change| Rent', 'Recommendations - Change Rent')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendationsChangeRev', row, 'Recommendations | Change| Rev', 'Recommendations - Change Rev')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendationsRecentAvgEffRent', row, 'Recommendations | Recent Avg Eff| Rent', 'Recommendations - Recent Avg Eff Rent')),
      num(getValWithOverrides(PRICING_ALIAS, 'RecommendationsRecentAvgEffPercentage', row, 'Recommendations | Recent Avg Eff | %', 'Recommendations - Recent Avg Eff Percentage')),
